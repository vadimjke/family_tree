    ...
    for (let i = 0; i < personsListArray.length; i++) {
      ...
      personImageSrc = await page.evaluate(
        (el) =>
          el
            .querySelector(
              "td.body_txt > table > tbody > tr > td > center > img"
            )
            .getAttribute("src"),
        personPersonalPage
      );
      ...
      const personAwardLink = `${createLink(
        personsListArray[i].personHREF
      ).slice(0, -7)}/awards/i.html`;
          ...
          .forEach((el) => {
            ...
            let href =
              className === "person2"
                ...
                : null;
            let id = href ? href.match(/\d+/)[0] : null;
            arr.push({ name, className, href, id });
          });
        return arr;
      });
      ...
      let personFamilyTiesArrayTransformed = [];

      for (let i = 0; i < personFamilyTiesArrayLinked.length; i++) {
        personFamilyTiesArrayLinked[i].forEach((el) => delete el.className);
        let familyTieName = personFamilyTiesArrayLinked[i].shift().name;
        let obj = {};
        obj.tieName = familyTieName;
        obj.persons = personFamilyTiesArrayLinked[i];
        personFamilyTiesArrayTransformed.push(obj);
      }

      personFamilyTies = personFamilyTiesArrayTransformed;
      ...
      const personBiographyFrame =
        await personBiographyNodeHandle.contentFrame();
